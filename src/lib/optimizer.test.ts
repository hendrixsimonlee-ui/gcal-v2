...
// People the AD has taken out of a dance for the week were being counted as
// attending, so the proposal claimed everyone was expected and the number
// only corrected itself once the drafts loaded into the week checklist.
{
  const slotWithExclusion = slot(0, "studio");
  slotWithExclusion.excludedCastMembers = [
    { userId: "b", name: "b", role: "DANCER", reason: null },
  ];

  const result = solveWeek({
    dances: [dance("Piece", ["a", "b", "c"], [slotWithExclusion])],
    maxRuns: 1,
  });

  assertEqual(result.placements[0]?.expectedCount, 2, "somebody taken out of the week is not counted as attending");
  assertEqual(result.placements[0]?.castSize, 3, "...while the cast size still counts them");
  assert(result.placements[0]?.missingUserIds.includes("b"), "...and they are named among the people missing");
  assertEqual(result.totalExpectedAttendance, 2, "...so the week's total headcount is the real one");
}
